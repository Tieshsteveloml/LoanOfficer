...
function Reviews({ editable = false, user = userData?.id, onlyPublished = false, data }) {
   ...
   const getReviews = async () => {
      ...
      result.forEach((item) => {
         item.published ? (item.reviewStatus = 1) : (item.reviewStatus = 0);
      });
      console.log("result");
      console.log(result);
   ...
   useEffect(() => {
      ...
      const total_pages =
         reminder > 0
            ? Math.floor(filteredReviews.length / ITEMS_PER_PAGE) + 1
            : Math.floor(filteredReviews.length / ITEMS_PER_PAGE);
   ...
   const setReview = (data) => {
      const index = ReviewsData.findIndex((x) => x.id === data.id);
      const newArray = [...ReviewsData.slice(0, index), { ...data }, ...ReviewsData.slice(index + 1)];
      ...
      <section className="reviews">
                           ...
                           <li key={item.id}>
                              <label>
                                 <input
                                    ...
                                    defaultChecked
                                    value={item.id}
                                    onClick={() => setFilterState(item.id)}
                                 />
         ...
         <div className="paginationWrap">
            {filteredReviews.length > 0 ? (
               <ul className="dflx pagination">
                  <li className={pageNumber === 1 ? `disable` : ``} onClick={() => setPageNumber(pageNumber - 1)}>
                     {" "}
                     PREVIOUS
                  </li>
                  ...
                  <li
                     className={pageNumber === totalPages ? `disable` : ``}
